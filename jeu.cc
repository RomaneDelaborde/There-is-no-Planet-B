...
void Jeu::lecture_csv_carte_objet(std::string nom_fichier) {
  ...
  if (fichier) {
    ...
    while (std::getline(fichier,ligne)) {
      ...
      else {_cartes_objets.push_back(Objet(nom_carte, id, est_objet_inventaire));} // sinon si l'objet n'est pas combinable avec d'autres objets (ex: certificat)
...
Objet Jeu::objet(const int id_carte) const {
  for (std::size_t i = 0; i < _cartes_objets.size(); i++) {
    if (_cartes_objets[i].get_id() == id_carte) {return _cartes_objets[i];} // si on a trouvé un match des id, on retourne l'objet correspondant
  }
...
Enigme Jeu::enigme(const int id_carte) const {
  for (std::size_t i=0; i< _cartes_enigmes.size(); i++) {
    if (_cartes_enigmes[i].get_id() == id_carte) {return _cartes_enigmes[i];} // si on a trouvé un match des id, on retourne l'énigme correspondante
  }
...
bool Jeu::id_existe(const int id_carte) const {
  if (_map_id.find(id_carte) == _map_id.end()) {return 0;} // si l'id n'existe pas dans _map_id
...
void Jeu::solution_enigme_valide(int id_carte_enigme, int val) {
  if (!std::count(_id_cartes_enigmes.begin(), _id_cartes_enigmes.end(), id_carte_enigme)) {
